...
function RentHouseFilter() {
  ...
  const { data: districtData, isLoading: districtIsLoading } = useQuery({ queryKey: ['districts'], queryFn: fetchAllDistricts });
  const { data: roomTypeData, isLoading: roomTypeIsLoading } = useQuery({ queryKey: ['roomTypes'], queryFn: fetchAllRoomTypes });
  ...
  if (cityData && districtData) {
    const cityMap = cityData.map(city => ({
      ...
      children: [
        ...
        ...districtData.filter(district => district.cityId === city.id).map(district => ({
  ...
  if (roomTypeData) {
    roomTypeOptions.push(...roomTypeData.map(roomType => ({
  ...
  return (
    <>
      <Form form={form} name="search" autoComplete="off" style={{ marginTop: '1rem' }}>
        <Row gutter={12}>
          <Col span={7}>
