    ...
    <Modal
      title={
        <div className='flex flex-col items-center'>
          ...
          <span className='font-semibold'>{t('priorityModal.confirmTitle')}</span>
        </div>
      }
      open={isVisible}
      onCancel={onCancel}
    ...
    >
      <div className='space-y-6'>
        <h3 className='mb-2 text-base font-medium'>
          {t('priorityModal.confirmMessage')} '<span className='text-blue-500'>{property.title}</span>' ?
        </h3>

        <div className='rounded-lg bg-gray-50 p-4'>
          <div className='mb-3 flex items-center text-sm text-gray-600'>
            <ClockCircleOutlined className='mr-2' />
            {t('priorityModal.lastPriorityExpiration')}: {lastPriorityExpiration}
          </div>
          <div className='space-y-2 text-sm'>
            <div className='flex items-start'>
              <ArrowUpOutlined className='mr-2 mt-1 text-green-500' />
              <span>{t('priorityModal.highlightOnHomepage')}</span>
            </div>
            <div className='flex items-start'>
              <ArrowUpOutlined className='mr-2 mt-1 text-green-500' />
              <span>{t('priorityModal.increaseVisibility')}</span>
            </div>

            <div className='flex items-start'>
              <span className='ml-6'>{t('priorityModal.priorityDuration')}</span>
            </div>
          </div>
        </div>
          ...
          <Tooltip
            title={
              <ul className='m-0 list-inside list-disc p-0'>
                <li>{t('priorityModal.policyHighlight1')}</li>
                <li>{t('priorityModal.policyHighlight2')}</li>
                <li>{t('priorityModal.policyHighlight3')}</li>
              </ul>
            }
          >
            <span className='cursor-help underline'>{t('priorityModal.policyNote')}</span>
          </Tooltip>
        </div>
